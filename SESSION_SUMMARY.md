...
## 2026-09-22 — 자동 감지 시뮬레이터 (브랜치 `workspace-mac-2`)

런타임 테스트 흐름("`/simulate/tick`으로 이벤트 생성 → 스캔 버튼 → Slack → 에이전트
탭에서 조회·승인 → CMMS 등록") 중 앞부분(이벤트 생성~Slack)을 실제 제조 설비 알람
구조에 맞게 자동화. 설계·구현 전 과정과 실측 근거는 별도 문서로 분리했다 —
자세한 내용은 **`SIMULATOR_PLAN.md` 참고**(이 항목은 요약만).

- **1단계**: `backend/data/sim_engine.py`(설비별 HEALTHY→DEGRADING→FAULT→고장+정비
  상태 머신), `sim_store.py`(전용 테이블 저장). 처음엔 임의로 설계했다가, 원본
  데이터셋(설비 100대·고장 761건·오류 3,919건)을 직접 측정해서 열화 강도·리드타임·
  오류-고장 상관관계를 실측치로 전부 교체(예: 고장 직전 편차가 처음 설계한 6σ가
  아니라 실제로는 평균 1.6σ). 회귀 테스트 10개, 코드를 일부러 8가지로 망가뜨려
  전부 잡히는지 변이 테스트까지 완료.
- **2단계**: `sim_query.py`(원본 읽기전용 데이터 + `sim_*` 데이터 통합 조회, 인덱스
  기반이라 전체 스캔 없음) + `pdm_telemetry`/`pdm_operations`/`event_store` 4개 함수
  연결. 기준선이 시뮬레이션 데이터에 오염되지 않는 것까지 복제 DB로 검증.
- **3단계**: `sim_loop.py` — FastAPI lifespan에 `asyncio` 백그라운드 태스크로 연결,
  60초=1시간 속도로 자동 전진, `sim_control`에 난수 상태를 영속화해서 서버 재시작 후에도
  같은 시나리오로 이어짐. `/simulator/start|stop|status|inject|reset` 5개 엔드포인트.
- **클린 상태 문제 발견**: 사용자 지적으로, 실제 DB에 이전 수동 테스트 잔재 이벤트
  3건이 남아있어서 자동 시뮬레이터가 새로 돌아도 안 지워진다는 걸 확인. 자동 삭제는
  안 하기로 결정(데이터 유실 방지) — `/simulator/reset`을 **명시적으로 호출**해야만
  `sim_*` 테이블과 `detected_events`/`completed_events`를 전부 지움.
- **4단계**: 3단계에 이미 포함(증분 스캔 + Slack 묶음 알림). 별도 작업 없이 완료
  처리. 리네이밍: `main.py`의 `/scan` 핸들러 이름이 새 `agent_service.scan_machines()`와
  겹쳐서 `trigger_scan()`으로 정리.
- **5단계**: `frontend/streamlit_app.py`에 `@st.fragment(run_every="10s")` 상태 패널
  추가(실행/정지, 시뮬레이션 시각, 열화 진행 바, 리셋 경고) — 기존 이벤트 목록 렌더링은
  건드리지 않고, 이벤트 개수가 바뀌었을 때만 전체 리런해서 갱신.
- **최종 검증**: Docker 재빌드 후 실제 컨테이너로 end-to-end 확인 — reset → 설비 #42
  강제 열화 → start → 약 50초 후 첫 틱 자동 실행(`sim_now` 06:00→07:00, 진행률
